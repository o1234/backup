...
BandwidthAvgInTimeAlgorithm::BandwidthAvgInTimeAlgorithm (const videoData &videoData,
                                                          const playbackData & playbackData,
                                                          const bufferData & bufferData,
                                                          const throughputData & throughput) :
  BandwidthAlgorithm (videoData, playbackData, bufferData, throughput),
  m_bandwidthAlgoIndex(1),//bandwidthAvgInTime(Average Of DeltaTime) = 1, bandwidthCrosslayer(get From PHYlayer) = 2,
                          //bandwidthLongAvg(long-Term Average) = 3, bandwdithAvgInChunk(short-Term Average) = 4
  m_deltaTime(5000000),//5s
                                     ...
                                     int64_t extraParameter2)//reservation
{
  ...
  answer.bandwidthEstimate = (int64_t) AverageBandwidth (timeNow - m_deltaTime, timeNow, answer.decisionCase) ;
  return answer;
}
double
BandwidthAvgInTimeAlgorithm::AverageBandwidth (int64_t t_1, int64_t t_2, int64_t& decisioncase)
{
  if (t_1 < 0)
  {
    t_1 = 0;
  ...
  {
    if (m_throughput.transmissionStart.at(i) < t_1) continue;
    else
      {
        indexStart = i;
        break;
  ...
  {
    if(m_throughput.transmissionEnd.at(j)>t_2) continue;
    else
    {
      indexEnd = j;
      break;
  ...
  {
    sumThroughput += 8 * m_videoData.segmentSize.at(m_videoData.userInfo.at(indexStart)).at(m_videoData.repIndex.at(indexStart)).at(indexStart);
    transmissionTime += (double)(m_throughput.transmissionEnd.at(indexStart) - m_throughput.transmissionStart.at(indexStart)) / 1000000.0;
    indexStart ++;
  }
  decisioncase = 1;
