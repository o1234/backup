 ...
 *  \ingroup tcpStream
 *  \brief This struct contains the reply an bufferDrop algorithm returns to the client
 */
struct bufferAlgoReply//new for buffer flush algo, added in 12-24
{
  ...
  int64_t bufferCleanNumber;//!< number of buffered segment to be cleaned (in Reverse order), only makes sense when bufferclean is choosen ( bufferClean = 1)
  int64_t bufferTargetNumber;//!< time in microsends for target buffer, default value is set with each bufferAlgo (in private data);
  int64_t decisionTime; //!< time in microsends when the buffer algorithm works, only for logging purposes
};
...
{
  int64_t bandwidthAlgoIndex;//!< bandwidthAvg = 1 bandwidthCrosslayer = 2 default = 0
  double  bandwidthEstimate;//!< bandwidth estimation value
  int64_t decisionTime; //!< time in microsends when the bandwidth algorithm works, only for logging purposes
  int64_t decisionCase;//!<for BandwidthCrosslayer, decision = 0 ->BandwidthDefault, decision = 1->BandwidthCrosslayer
                                            // !<for BandwithAvg,decison = 0->0(not enough data), decison =1->BandwidthAvg
};
 ...
 *  \usually Only bufferData is used for dash, considering layer-based video coding, enhancement layer buffer may be required
 * Tracks the status of the buffer level.
 */
struct bufferData
{
	std::vector<int64_t> timeNow; //!< current simulation time
  std::vector<int64_t> segmentIndex; //!< segmentIndex of segment in buffer(base layer) -1 = fail
...
struct bufferEnhance1//new for layer-based video coding, added in 12-24
{
  ...
  std::vector<int64_t> segmentIndex; //!< segmentIndex of segment in buffer(Enhancement layer1) -1 = fail
	std::vector<int64_t> bufferLevelOldE1; //!< buffer level in microseconds before adding segment duration (in microseconds) of just downloaded segment
};
...
struct bufferEnhance2//new for layer-based video coding, added in 12- 24
{
	...
	std::vector<int64_t> segmentIndex; //!< segmentIndex of segment in buffer(Enhancement layer2) -1 = fail
	std::vector<int64_t> bufferLevelOldE2; //!< buffer level in microseconds before adding segment duration (in microseconds) of just downloaded segment
	std::vector<int64_t> bufferLevelNewE2; //!< buffer level in microseconds after adding segment duration (in microseconds) of just downloaded segment
};
...
struct videoData
{
	std::vector < std::vector < std::vector<int64_t > > > segmentSize; //!< vector holding representation levels in the first dimension and their particular segment sizes in bytes in the second dimension
        // 2-D vector, < viewPoint < RepsLevel <segmentsize>>
	std::vector <std::vector < double > > averageBitrate; //!< holding the average bitrate of a segment in representation i in bits
        // 2-D vector, < viewPoint < RepsLevel's Bitrate >
	int64_t segmentDuration; //!< duration of a segment in microseconds
  std::vector <int64_t> repIndex;
  std::vector <int64_t> userInfo;
};
